...
<?php
	...
	foreach ($result as $row) {
		...
		unlink('../assets/images/partenaire/'.$p_featured_photo);
	}
	...
	foreach ($result as $row) {
		$photo = $row['photo'];
		unlink('../assets/images/partenaire/photo/'.$photo);
	}
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_order WHERE service_id=?");
	$statement->execute(array($_REQUEST['id']));
	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
	foreach ($result as $row) {
	...
	$statement = $pdo->prepare("DELETE FROM tbl_order WHERE service_id=?");
	$statement->execute(array($_REQUEST['id']));
